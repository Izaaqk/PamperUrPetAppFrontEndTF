...
export class RegisterReservaComponent implements OnInit{
  ...
  aceptar() {
    ...
    if (this.form.valid) {
      ...
      this.http.post<any>(`http://localhost:8080/api/reserva/${this.form.value['idPagoSeleccionado']}/${this.form.value['idPaseadorSeleccionado']}/${this.form.value['idPropietarioSeleccionado']}/${this.form.value['idMascotaSeleccionado']}`, reservaData)
        .subscribe((data) => {
